      ...
      <div className="flex flex-col md:flex-row mx-auto items-center max-w-4xl w-[95vw] w-full p-8 rounded-2xl" style={{
        ...
        <Delay delay={0}>
          <div className="md:w-1/2 mb-5 md:text-left pr-6 slideUp">
            <LazyLoadImage src={'/flag.png'} effect='opacity' alt="Agrosmart Logo" about='Agrosmart Logo' className='' style={{
              position: 'fixed',
...
function GetStartedMini({setter}) {
  return (<div className="sudo w-[80vw] lg:w-2/3  text-lightAf  text-center slideUp" style={{
    zIndex: 1
  }}>
    <h2 className="text-xl fs-2 mb-3 font-semibold">Welcome</h2>
    <p className="text-gray-500 text-sm mt-1">Log in to continue</p>

    <button className="mt-4 flex items-center themebg justify-center w-full px-4 py-2 border rounded-lg text-gray-700 shadow-sm text-sm font-medium" onClick={() => {
    ...
    <Link onClick={()=>setter('max')} className="flex themebg items-center border bg-none justify-center w-full px-4 py-2 rounded-lg text-gray-700 shadow-sm text-sm font-medium no-dec text-dark">
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        <path strokeLinecap="round" strokeLinejoin="round" d="M16 12H8m8 4H8m8-8H8m12 4c0 4.418-3.582 8-8 8s-8-3.582-8-8 3.582-8 8-8 8 3.582 8 8z" />
      </svg>
      Sign in with Email
    </Link>

    <p className="mt-4 text-sm text-gray-500">
      Dont have an account? <a href="https://usgeaan.onrender.com/signup" className="text-blue-600 text-dark no-dec">Sign up</a>
    </p>
  </div>)
}
...
const LoginWithEmail = ({setter}) => {
    ...
    useEffect(() => {
        ...
        const t = ((location.href.split('?')[1] || '').split('=')[1] || '').split('&')[0]
        if (t) {
    ...
    return (
            <div className="w-full max-w-md p-8 text-lightAf rounded-lg slideUp" style={{zIndex:3}}>
